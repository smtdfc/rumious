...
export default function rumious(options = {}) {
  ...
  const configFilePath = path.join(
    cwd,
    options.configFile || 'rumious.configs.json',
  )
    ...
    resolveId(source, importer) {
      if (/\.(png|jpg|jpeg|gif|svg|mp3|mp4|wav|ogg|webm)$/i.test(source)) {
        if (!configs.allowImportMediaFiles) {
          this.error({
            message:
              'Importing media files is not allowed. Please enable the "allowImportMediaFiles" option in your configuration.',
      ...
      if (source.endsWith('.rumious')) {
        if (!configs.allowRumiousFiles) {
          this.error({
            message:
              'Importing .rumious files is not allowed. Please enable the "allowRumiousFiles" option in your configuration.',
            plugin: 'rollup-plugin-rumious',
          })
        }
        return path.resolve(path.dirname(importer), source)
