...
export class RouterModule extends Module {
  private routeData: RouteData = {
    params: createState<Record<string, string>>({}),
    query: null,
  };
  ...
  diffLayout(layouts: RouteComponent[]): number {
    for (
      let i = 0;
      i < Math.max(this.currentLayouts.length, layouts.length);
      i++
    ) {
  ...
  async resolvePage(path: string) {
    ...
    const walk = (segments: string[], routes: RouteConfig[]): boolean => {
    ...
    if (changePos != -1 && layouts.length > 0) {
      ...
      if (addition > 0) {
        this.slots.push(
          ...Array.from({ length: addition }).map(() => createState(null)),
        );
      }
